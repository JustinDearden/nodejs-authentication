...
router.post("/logout", (req, res) => {
  ...
  try {
    ...
    // Delete the session token from Redis for the corresponding user
    redisClient.del(`session:${payload.userId}`, (err) => {
      ...
      // Log successful logout and send a success response
      logger.info(`User logged out successfully: ${payload.username}`);
      return res.status(200).json({ message: "Logout successful." });
    });
  } catch (error) {
    // If token verification fails, log a warning and respond with an error
